...
export class GenerationService {
  ...
  private buildResolvedInput(
    ...
    const style = QUESTION_STYLES[
      Math.floor(
        attempt /
          (categoryCount * TOPIC_VARIANTS.length * CONTEXT_FRAMES.length)
      ) % QUESTION_STYLES.length
    ];
  ...
  private async generateAndStoreWithResult(
    ...
    const uniquenessKey = this.buildUniquenessKey("quiz", responsePayload, language);
